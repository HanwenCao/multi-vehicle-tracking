...
'''
重要 包含3个函数（初始化、前向计算、加载参数）
self.base = Resnet网络模型
self.gap = GeM()
self.bottleneck = BN层
self.classifier = 分类器
...
class Baseline(nn.Module):
    ...
    def __init__(self, num_classes, last_stride, model_path, neck, neck_feat, model_name, pretrain_choice, cfg):
        super(Baseline, self).__init__()  #测试时model_name=resnet50_ibn_a
        self.base = build_backbone(model_name, last_stride) # 创建resnet50ibna模型（是否这一步不加载预训练参数？）
        if 'regnet' in model_name:
            self.in_planes = self.base.in_planes

        if pretrain_choice == 'imagenet':  # 测试时MODEL.PRETRAIN_CHOICE "('self')"不加载ImageNet的参数给resnet50ibna，后续在大main会加载reid训练好的参数
            self.base.load_param(model_path)
            print('Loading pretrained ImageNet model......')

        if cfg.MODEL.POOLING_METHOD == 'GeM':  # GeM pooling
            print('using GeM pooling')
            self.gap = GeM()# GeM pooling
        else:
            self.gap = nn.AdaptiveAvgPool2d(1) #普通avg pooling

        self.num_classes = num_classes  #重要参数，大main创建loader时得到
        self.neck = neck  #cfg.MODEL.NECK 是啥？
        self.neck_feat = neck_feat  #cfg.TEST.NECK_FEAT 是啥？
        self.ID_LOSS_TYPE = cfg.MODEL.ID_LOSS_TYPE  # 决定self.classifier类型 是啥？

        self.bottleneck = nn.BatchNorm1d(self.in_planes)  # Batch Norm（dim=2048）
        self.bottleneck.bias.requires_grad_(False)  # no shift  不训练Batch Norm的bias
        ...
        self.bottleneck.apply(weights_init_kaiming)  #初始化BN层？
        self.classifier.apply(weights_init_classifier)  #初始化classifier？
        #self.att = SpatialAttention2d(2048, 512)
        '''大初始化结束'''

    def forward(self, x, label=None, return_featmap=False):

        featmap = self.base(x)  # (b, 2048, 1, 1) 经过Resnet
        #featmap = self.bottleneck(featmap)
        #featmap = self.att(featmap) * featmap
        if return_featmap:
            return featmap
        global_feat = self.gap(featmap)  #经过Gem pooling
        #global_feat = global_feat.view(global_feat.shape[0], -1)  # flatten to (bs, 2048)
        global_feat = global_feat.flatten(1)  #变形

        feat = self.bottleneck(global_feat)  # normalize for angular softmax 经过BN层
        if self.training:
            if self.ID_LOSS_TYPE in ('arcface', 'cosface', 'amsoftmax', 'circle'):
                cls_score = self.classifier(feat, label)
            else:
    ...
    def load_param(self, trained_path, skip_fc=True):  #加载reid训练得到的参数（跳过classifier）
        try:
            param_dict = torch.load(trained_path).state_dict()  #问题：和torch.load(trained_path)有什么区别？
        except:
            param_dict = torch.load(trained_path)
        for i in param_dict:
            if skip_fc and 'classifier' in i:
            ...
            if self.state_dict()[i].shape != param_dict[i].shape:
                print('skip {}, shape dismatch {} vs {}'.format(i, self.state_dict()[i].shape, param_dict[i].shape))
                continue
            self.state_dict()[i].copy_(param_dict[i])  #参数从param_dict => self.state_dict()