...
class ImageDataManager(BaseDataManager):
    ...
    def __init__(self,
        ...
        for name in self.train_sets:  # --train_sets Aic        #--root ./datasets
            dataset = init_imgreid_dataset(
                root=self.root, name=name)  # /identifier/datasets/aic.py
            # 注意pid命名规则
            # camid命名规则：真正的camid-1（c033变为camid=32）
...
#                 print(img_path, pid, camid)
        ...
        print('build training DataLoader')
#         mean, std = calculate_mean_and_std(self.trainloader, len(train))   #too slow
