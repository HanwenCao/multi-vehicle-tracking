...
def argument_parser():
    ...
    parser.add_argument('--tracklet_path', type=str, default="./exp/tracklets_mask.txt",
    ...
    parser.add_argument('--output_path', type=str, default="/data/caohw9/track3_intermediate/aic_test_bigimg",
                        help='path to the output dictionaries')
...
def extract_im_api(args): #args=[数据集地址，输出query和gallery地址，"test",S06，camID，跟踪结果]
    ...
    scene = args[3]
    cam = args[4]
    gts = args[5]  #gts包含[camID ID号 帧号 位置信息]
        ...
        if fr_id not in frames or im is None: #如果视频当前帧没有任何跟踪结果：下一帧
            ...
            for track in tracks: #1个[left,top,right,bot,car_id,query]
                ...
                if query:
                    ...
                    #为了防止某一个track只有一张图，把它同时放入query和gallery，但是后面要小心reid时这个query很优先从gallery中找到他自己
    ...
    for split in splits:
        ...
        for scene in scenes:
            ...
            for cam in cams:#遍历test集的所有相机（视频）
                ...
                for track in or_tracks: #取出tracklets的每一行（重要）
                    if int(track.split(" ")[0]) == camid: #找到对应这个camID的所有跟踪结果，记录在gts里
                        #补充图片尺寸条件
                        w = int(track.split(" ")[5])
                        h = int(track.split(" ")[6])
                        if w*h>=9000:
                            gts.append(track)  #gts包含[camID ID号 帧号 位置信息]当前相机下的某一检测跟踪结果放入这个相机的gts中
                ...
                #args_list是对tracklets信息按照相机分类，属于一个相机的所有检测跟踪放在一起
                #len(args_list)=6 args_list[i-1]分别是base_path,data_path,'test','S06','c04i',gts
                #其中gts包含tracklets中所有属于这个cam的行，如'41 15 0 1088 134 22 15 0 0'...
    n_jobs = args.njobs
    pool = Pool(n_jobs)
    pool.map(extract_im_api, args_list) #把args_list送入提取图片的程序(多进程，感觉是对不同视频多进程处理)
    pool.close()
